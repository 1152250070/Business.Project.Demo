...
namespace Business.Project.Demo.Framework
{
    ...
    public class IdentifyEncoding
    {
        ...
        internal static string[] niceName = new string[]{"GB2312", "GBK", "HZ", "Big5", "CNS 11643"
    , "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER"};
        ...
        /// </summary>
        /// <param name="fullName">要判断的 <see cref="Uri"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        public string GetEncodingName(System.Uri testurl)
        {
            sbyte[] rawtext = new sbyte[10240];
            int read = 0, offset = 0;
            try
            {
                Stream cs = WebRequest.Create(testurl.AbsoluteUri).GetResponse().GetResponseStream();
                while ((read = ReadInput(cs, ref rawtext, offset, rawtext.Length - offset)) > 0)
                {
                    offset += read;
                }
                cs.Close();
        ...
        public Encoding GetEncoding(System.Uri fileName)
        {
            string enName = GetEncodingName(fileName);
            try
            {
                return Encoding.GetEncoding(enName);
            }
            catch { }
        ...
        /// <summary>
        /// 从指定的 FileInfo 中判断编码类型
        /// </summary>
        /// <param name="file">要判断的 <see cref="FileInfo"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        public string GetEncodingName(FileInfo file)
        {
            sbyte[] rawText = new sbyte[(int)FileLength(file)];
            try
            {
                FileStream cs = new FileStream(file.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite);
                ReadInput(cs, ref rawText, 0, rawText.Length);
            }
            catch (System.Exception e)
            {
        ...
        public virtual string GetEncodingName(sbyte[] rawText)
        {
            ...
            //Encoding.GetEncoding 无此类型编码，会报异常这里取消判断
        ...
        internal virtual int UTF8Probability(sbyte[] rawtext)
        {
            ...
            for (int i = 0; i < rawLength; i++)
            {
                ...
                else if (-64 <= rawtext[i] && rawtext[i] <= -33 && i + 1 < rawLength && -128 <= rawtext[i + 1] && rawtext[i + 1] <= -65)
                {
                    goodbytes += 2;
                    i++;
                }
                else if (-32 <= rawtext[i] && rawtext[i] <= -17 && i + 2 < rawLength && -128 <= rawtext[i + 1] && rawtext[i + 1] <= -65 && -128 <= rawtext[i + 2] && rawtext[i + 2] <= -65)
                {
                    goodbytes += 3;
                    i += 2;
                }
            ...
            if (asciibytes == rawLength)
                return 0;

            score = (int)(100 * ((float)goodbytes / (float)(rawLength - asciibytes)));
            // If not above 98, reduce to zero to prevent coincidental matches
            // Allows for some (few) bad formed sequences
            return ((score > 98) || (score > 95 && goodbytes > 30)) ? score : 0;
        }
        #endregion
        ...
        {
            return (((sbyte)Identity(0xFE) == rawtext[0] && (sbyte)Identity(0xFF) == rawtext[1]) || ((sbyte)Identity(0xFF) == rawtext[0] && (sbyte)Identity(0xFE) == rawtext[1]))
                ? 100 : 0;
        }
        #endregion
        ...
        /// 检测当前文件的大小
        /// </summary>
        /// <param name="fullName">被检测的文件</param>
        /// <returns>当前文件的大小。</returns>
        public static long FileLength(FileInfo file)
        {
            return (Directory.Exists(file.FullName)) ? 0 : file.Length;
        }
        /// <summary>
